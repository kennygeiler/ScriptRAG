...
| **3** | **Phase 2 shipped:** **Haiku-first** for bundled **semantic auditors** (`call_audit_llm_with_usage`); **Sonnet** on failure; extract/fixer stay **Sonnet → Haiku**. **`PIPELINE_TELEMETRY_VERSION = 3`**. Append a **Results log** row when you benchmark v2 vs v3 on the same script/range. |
| **4+** | Phase 3+ (conditional audit, caching, …) — bump **`PIPELINE_TELEMETRY_VERSION`** when storage/attribution contract changes again. |
